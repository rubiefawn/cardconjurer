...
function applyPresetColor(index, mode) {
    ...
    // Always use the base opacity (0.2) plus offset for square 2, regardless of current values
    if (index === 2 && !card.station.disableFirstAbility) {
        card.station.squares[2].opacity = Math.min(1.0, 0.2 + opacityOffset); // Use base 0.2 instead of current opacity
        ...
        card.station.squares[2].opacity = 0.2; // Reset to base opacity
